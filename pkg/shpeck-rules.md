...
These rules are for AI agents working in this repo.

This repo uses **Shpeck**: a local-only, spec-driven development (SDD) workflow for brownfield codebases.

Shpeck introduces local artifacts (primarily under `.spec/`) that help an agent move from intent -> research -> spec -> plan -> code -> verification -> PR explanation, while keeping scope tight and respecting existing codebase conventions.

These artifacts are intentionally local-only and are not committed.
...
- `.spec/<context>/context.toml`: context metadata (type, and ticket key for ticket contexts).
...
- **Init is required:** if `.shpeck.toml` or `.spec/` is missing, the repo has not been initialized; the user must run `shpeck init` before agent commands that expect Shpeck state.
- **Local-only means local-only:** never suggest staging/committing or including `.spec/`, `.shpeck.toml`, or tool config dirs (e.g. `.opencode/`, `.claude/`) in a PR.
- **Intent authority is constrained:** in ticket contexts, `ticket.md` defines intent; `spec.md` defines implementation details and MUST NOT change intent/acceptance relative to `ticket.md`. If intent needs to change, update `ticket.md` first (and the external ticket if available) before continuing.
...
- **No invented commit workflow:** Shpeck does not manage commits; do not invent commit rules beyond what the user/project asks for.
